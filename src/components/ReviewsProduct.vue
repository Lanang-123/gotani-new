<template>
    <div class="container md:mx-auto mx-6 md:-mt-0 -mt-32">
        <div class="md:max-w-3xl ">
            ...
            <div v-for="comment in productComments" :key="comment.id" class="border-2 border-amber-300 rounded-md p-4 mb-4 ">
                    ...
                    <div>
                        ...
                        <div class="flex justify-center items-center">
                          ...
                          />
                            ...
                            <p class="text-sm text-gray-600 italic ml-2">{{ formatDate(comment.user.updated_at) }}</p>
                        </div>
                    </div>
                </div>
                <p>{{ comment.comment }}</p>
            ...
            <form @submit.prevent="addComment" class="bg-white shadow-md rounded p-4">
                <div class="mb-4">
                        ...
                        <i class="text-amber-500">Bisa masukkan angka desimal dari 1.0 - 5.0</i>
                        <input type="text" v-model="newComment.rating" id="rating"
                            class="w-full px-3 py-2 border border-gray-300 rounded">
                    </div>
                <div class="mb-4">
                    <label for="content" class="block font-bold mb-2">Komentar:</label>
                    <textarea v-model="newComment.content" id="comment"
                        class="w-full px-3 py-2 border border-gray-300 rounded"></textarea>
                </div>
                <div>
                    <button type="submit"
                        class="bg-amber-500 hover:bg-amber-700 text-white font-bold py-2 px-4 rounded">Kirim</button>
                </div>
            </form>
        </div>
    </div>
...
const getActiveUser = async () => {
    ...
    if(token) {
        try {
            ...
            newComment.value.id_user = activeUser.id;
        } catch (error) {
            console.log(error);
        }
    }
...
onMounted( async ()=>{
    idProduct.value = props.productId;
...
const addComment = async () => {
    ...
    if(meToken) {
        try {
            ...
            const process = await axios.post("http://localhost:8000/api/reviews", {
                id_product: idProduct.value,
                comment: newComment.value.content,
                rating: newComment.value.rating,
                id_user: newComment.value.id_user
            }, { headers });
