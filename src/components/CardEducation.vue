    ...
    <div>
        ...
        <div class="px-4 py-16 mx-auto max-w-full md:px-8 lg:pt-20">
            <div class="max-w-xl mb-10 md:mx-auto sm:text-center lg:max-w-2xl md:mb-2">
              <h2 class="max-w-lg mb-6 font-sans text-3xl font-bold leading-none tracking-tight text-[#245953] sm:text-4xl md:mx-auto">
              ...
              <div v-for="category in categorys" :key="category.id">
                <button @click="clickCategory(category.id)" >
                  <div class="inline-block overflow-hidden duration-300 transform bg-white rounded shadow-sm hover:-translate-y-2">
                      ...
                      <img :src="urlCategoryEdu + 'images/' + category.images" class="object-cover w-full h-48" :alt="category.name_education" />
        ...
        <div class="px-4 mx-auto sm:max-w-full md:max-w-full lg:max-w-full md:px-24 lg:px-8">
            <h2 class="max-w-md text-2xl mb-6 font-sans md:text-2xl font-bold tracking-tight text-[#245953] sm:leading-none xl:max-w-full">
                Edukasi Pilihan
            </h2>
        </div>
            ...
            <article class="flex flex-col md:flex-row bg-white transition shadow-xl">
              <div class="sm:basis-56">
                <video class="aspect-square h-36 md:h-full w-full object-cover" controls>
                  <source :src="urlEducation + 'video/' + education.video_path" type="video/mp4">
                </video>
              </div>
              ...
              <div class="flex flex-1 flex-col justify-between">
                ...
                <div class="sm:flex sm:items-end sm:justify-end">
                  <RouterLink
                    ...
                    class="block bg-[#06B3B9] px-5 py-3 text-center text-xs font-bold uppercase transition hover:bg-[#245953] text-white"
                  >
                    Lihat Detail
                  </RouterLink>
                </div>
...
const urlEducation = 'http://localhost:8000/api/education/'
const urlCategoryEdu = 'http://localhost:8000/api/category-edu/'

const fetchDataEducation = (index) => {
  if(index) {
    axios.get(urlEducation + 'category/' + index)
...
const fetchCategoryEdu = () => {
  axios.get(urlCategoryEdu)
    .then((res) => categorys.value = res.data.data)
    .catch((err) => console.log(err));
}
